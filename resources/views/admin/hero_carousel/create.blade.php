...
@section('page-title', 'Add Hero Carousel Banner')

@section('content')
<div class="max-w-4xl mx-auto p-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Add Hero Carousel Slide</h2>
            <p class="text-sm text-gray-500 mt-1">Upload a new banner image or choose from presets to display on your homepage hero slider.</p>
        </div>
    ...
    <form action="{{ route('admin.hero_carousel.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
        @csrf

        <!-- Banner Image Section -->
        <div class="border-b border-gray-100 pb-6">
            <h3 class="text-base font-bold text-gray-900 mb-2 flex items-center gap-2">
                <i class="fas fa-image text-amber-600"></i> Step 1: Choose or Upload Banner Image
            </h3>
            
            <!-- Live Preview -->
            <div class="mb-4">
                <label class="block text-xs font-semibold text-gray-500 uppercase mb-1.5">Live Preview</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <div class="p-4 bg-gray-50 rounded-xl border border-gray-200">
                    <label class="block text-sm font-bold text-gray-800 mb-1">
                        <i class="fas fa-upload text-blue-600 mr-1"></i> Upload New Image File
                    </label>
                    <p class="text-xs text-gray-500 mb-3">Recommended size: 1920x600px or wide banner (JPG, PNG, WebP up to 10MB)</p>
                ...
                <div class="p-4 bg-gray-50 rounded-xl border border-gray-200">
                    <label class="block text-sm font-bold text-gray-800 mb-1">
                        <i class="fas fa-th-large text-green-600 mr-1"></i> Or Pick from Available Banners
                    </label>
                    ...
                    <select name="preset_image" id="presetSelect" class="w-full bg-white border border-gray-300 rounded-lg p-2 text-xs font-medium focus:ring-2 focus:ring-amber-500">
                        <option value="">-- Or choose a preset image --</option>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="button_text" id="buttonTextInput" value="{{ old('button_text', 'Order Now') }}" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-amber-500" placeholder="e.g., Order Now, Shop Now">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="link_url" id="linkUrlInput" value="{{ old('link_url', '/quotation') }}" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-amber-500" placeholder="/quotation or /combos">
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', 1) }}" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-amber-500" min="0">
                </div>
            </div>

            <div class="pt-2">
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="h-4 w-4 text-amber-600 focus:ring-amber-500 rounded border-gray-300">
        ...
        <div class="pt-4 border-t border-gray-100 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="bg-amber-600 hover:bg-amber-700 text-white font-semibold px-6 py-2.5 rounded-lg shadow transition text-sm flex items-center gap-1.5">
                <i class="fas fa-save"></i> Save Carousel Slide
            </button>
        </div>
    </form>
</div>
