    ...
    <div class="p-6 bg-white rounded-lg shadow-md">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            ...
            <div class="flex flex-col sm:flex-row gap-2 mt-4 sm:mt-0">
                ...
                <a href="{{ route('admin.coupons.export-csv') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                    <i class="fas fa-download"></i>
                    Export CSV
                </a>
                <a href="{{ route('admin.coupons.download-template') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div class="flex flex-col">
                                    <span class="font-medium">{{ $coupon->usage_display }}</span>
                                    @if($coupon->hasReachedUsageLimit())
                                        <span class="text-red-600 text-xs font-medium">Limit Reached</span>
                                    @elseif($coupon->usage_limit && $coupon->usage_limit > 0)
                                        <span class="text-green-600 text-xs">{{ $coupon->usage_limit - $coupon->used_count }} remaining</span>
                                    @else
                                        <span class="text-gray-500 text-xs">Unlimited usage</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $status = $coupon->status_with_usage;
                                @endphp
                                @if($status === 'Active')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                        <i class="fas fa-check-circle mr-1"></i>Active
                                    </span>
                                @elseif($status === 'Limit Reached')
