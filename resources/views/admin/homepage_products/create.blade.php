...
<div class="max-w-4xl mx-auto py-6 px-4 sm:px-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Add Home Page Product</h1>
            <p class="text-sm text-gray-500 mt-1">Select an existing product from inventory to auto-fill everything, or enter product details manually.</p>
        </div>
    ...
    <!-- MAIN FORM -->
    <form action="{{ route('admin.homepage_products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
        @csrf
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="item_name" name="item_name" value="{{ old('item_name') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 @error('item_name') border-red-500 @enderror" placeholder="Enter product name" required>
                </div>
                <div>
            ...
            <div class="mt-4">
                ...
                <textarea id="description" name="description" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" placeholder="Product description or packing details (e.g. 1 Box / 10 Pcs)">{{ old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="border-t border-gray-100 pt-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" id="original_price" name="original_price" value="{{ old('original_price') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" placeholder="0.00">
                </div>
                <div>
                    <label for="discount_percentage" class="block text-xs font-bold text-gray-700 uppercase mb-1">Main Disc (%)</label>
                    <input type="number" id="discount_percentage" name="discount_percentage" value="{{ old('discount_percentage', 70) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" placeholder="70">
                </div>
                <div>
                    <label for="special_discount_percentage" class="block text-xs font-bold text-gray-700 uppercase mb-1">Special Disc (%)</label>
                    <input type="number" id="special_discount_percentage" name="special_discount_percentage" value="{{ old('special_discount_percentage', 15) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" placeholder="15">
                </div>
                <div>
                    <label for="price" class="block text-xs font-bold text-amber-700 uppercase mb-1">Final Price (₹) *</label>
                    <input type="number" step="0.01" id="price" name="price" value="{{ old('price') }}" class="w-full px-3 py-2 border-2 border-amber-500 rounded-lg text-sm font-bold text-amber-900 bg-amber-50/50 focus:ring-2 focus:ring-amber-500" placeholder="0.00" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                <div>
                    ...
                    <input type="number" id="quantity" name="quantity" value="{{ old('quantity', 10) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" placeholder="0">
                </div>
                <div>
                    <label for="youtube_url" class="block text-xs font-bold text-gray-700 uppercase mb-1">YouTube Video URL</label>
                    <input type="text" id="youtube_url" name="youtube_url" value="{{ old('youtube_url') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" placeholder="https://youtube.com/watch?v=...">
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-gray-100 pt-5">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                ...
                <div class="w-32 h-32 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex items-center justify-center overflow-hidden flex-shrink-0 relative">
                    <img id="imagePreview" src="{{ asset('images/firework-default.png') }}" alt="Preview" class="w-full h-full object-contain p-2">
                    <span id="previewTag" class="absolute bottom-1 right-1 bg-gray-800 text-white text-[10px] px-1.5 py-0.5 rounded opacity-80">Default</span>
                </div>
        ...
        <div class="border-t border-gray-100 pt-5 bg-amber-50/50 -mx-6 -mb-6 p-6 rounded-b-xl">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                ...
                <label class="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 cursor-pointer hover:border-amber-400 transition">
                    <input type="checkbox" id="is_popular" name="is_popular" value="1" {{ old('is_popular') ? 'checked' : '' }} class="h-5 w-5 text-amber-600 focus:ring-amber-500 border-gray-300 rounded">
                    <div>
                ...
                <label class="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 cursor-pointer hover:border-blue-400 transition">
                    <input type="checkbox" id="is_latest" name="is_latest" value="1" {{ old('is_latest') ? 'checked' : '' }} class="h-5 w-5 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                    <div>
                        <span class="text-sm font-bold text-gray-800 flex items-center gap-1">
                ...
                <label class="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 cursor-pointer hover:border-green-400 transition">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="h-5 w-5 text-green-600 focus:ring-green-500 border-gray-300 rounded">
                    <div>
                        <span class="text-sm font-bold text-gray-800 flex items-center gap-1">
            ...
            <div class="flex items-center justify-end gap-3 pt-3 border-t border-amber-200">
                ...
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-bold px-6 py-2.5 rounded-lg shadow transition text-sm flex items-center gap-1.5">
                    <i class="fas fa-check"></i> Add Product to Home Page
                </button>
            </div>
        </div>
    </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const popularInput = document.getElementById('is_popular');
    const latestInput = document.getElementById('is_latest');
    ...
    quickSelect.addEventListener('change', function() {
        ...
        // If marked popular / latest in stock, tick them
        if (selected.dataset.popular === '1') {
            popularInput.checked = true;
        }
        if (selected.dataset.latest === '1') {
    ...
    // Auto-calculate final price if original price or discounts change
