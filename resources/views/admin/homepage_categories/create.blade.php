...
@section('page-title', 'Add Category Card (2026)')
...
<div class="max-w-4xl mx-auto py-6 px-4 sm:px-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            <div class="flex items-center gap-2">
                <h1 class="text-2xl font-bold text-gray-900">Add Category Card (2026)</h1>
                <span class="bg-amber-100 text-amber-800 text-xs px-2.5 py-0.5 rounded-full font-bold">2026 Showcase</span>
            </div>
            <p class="text-sm text-gray-500 mt-1">Add a new category card to the "Best For Your Categories" showcase on the home page.</p>
        </div>
    ...
    <form action="{{ route('admin.homepage_categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf

        <!-- Hidden input for selected gallery image path -->
        <input type="hidden" name="selected_image" id="selected_image_input" value="{{ old('selected_image', $availableImages[0]['path'] ?? '') }}">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-3">
                <div>
                    ...
                    <p class="text-xs text-gray-500">Click any current 2026 product image to assign it to this category card.</p>
                </div>
                <div class="text-right">
                    <span class="text-xs text-gray-400">Currently selected:</span>
                    <span id="currentSelectionLabel" class="block text-xs font-bold text-amber-700 truncate max-w-[200px]">{{ basename($availableImages[0]['path'] ?? '') }}</span>
                </div>
            </div>
            ...
            <div class="mb-5 p-4 bg-amber-50/50 rounded-xl border border-amber-200 flex items-center gap-4">
                <div class="w-24 h-24 bg-white rounded-lg p-2 border border-gray-200 shadow-sm flex items-center justify-center flex-shrink-0">
                    <img id="activeImagePreview" src="{{ $availableImages[0]['url'] ?? asset('images/firework-default.png') }}" alt="Preview" class="max-h-full max-w-full object-contain">
                </div>
                <div>
                    <span class="bg-green-600 text-white text-[11px] px-2 py-0.5 rounded font-bold">Selected Image Preview</span>
                    <h3 id="previewTitle" class="font-bold text-gray-900 mt-1 text-sm">{{ $availableImages[0]['title'] ?? 'Selected Product' }}</h3>
                    <p id="previewPath" class="text-xs text-gray-500 font-mono mt-0.5 break-all">{{ $availableImages[0]['path'] ?? '' }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 max-h-96 overflow-y-auto p-1 scrollbar-thin">
                @foreach($availableImages as $idx => $img)
                    @php
                        $isSelected = (old('selected_image', $availableImages[0]['path'] ?? '') == $img['path']);
                    @endphp
            ...
            <div class="mt-5 pt-4 border-t border-gray-200">
                ...
                <p class="text-[11px] text-gray-500 mt-1">Uploading a custom file will override the selected gallery image.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 font-semibold" placeholder="e.g. SINGLE FLASH" required>
                </div>

                <div>
                    <label for="search_term" class="block text-xs font-bold text-gray-700 uppercase mb-1">Search / Filter Tag</label>
                    <input type="text" id="search_term" name="search_term" value="{{ old('search_term') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" placeholder="e.g. SINGLE FLASH">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                <div>
                    ...
                    <input type="number" id="product_count" name="product_count" value="{{ old('product_count', 10) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500">
                    <p class="text-[11px] text-gray-500 mt-1">e.g. 10 Products</p>
                </div>

                <div>
                    <label for="sort_order" class="block text-xs font-bold text-gray-700 uppercase mb-1">Display Order #</label>
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $nextOrder) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500">
                    <p class="text-[11px] text-gray-500 mt-1">Display priority on home page</p>
                </div>

                <div class="flex flex-col justify-center space-y-2 pt-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="auto_count" value="1" {{ old('auto_count') ? 'checked' : '' }} class="h-4 w-4 text-amber-600 rounded">
                        <span class="text-xs font-bold text-gray-700">Auto-count active products</span>
                    </label>

                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="h-4 w-4 text-green-600 rounded">
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-bold px-6 py-2.5 rounded-lg shadow transition text-sm flex items-center gap-1.5">
                <i class="fas fa-plus-circle"></i> Create Category Card
            </button>
        </div>
    </form>
</div>
