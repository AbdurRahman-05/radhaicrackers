...
class CouponController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Handle usage limits: 0 means unlimited, null means unlimited
        // Only convert 0 to null if the field was explicitly provided and set to 0
        if ($request->filled('usage_limit') && $validated['usage_limit'] == 0) {
            $validated['usage_limit'] = null; // Convert 0 to null for unlimited
        }
        if ($request->filled('user_limit') && $validated['user_limit'] == 0) {
            $validated['user_limit'] = null; // Convert 0 to null for unlimited
        }
    ...
    // Export existing coupons to CSV
    public function exportCsv()
    {
        $filename = 'coupons_export_' . now()->format('Ymd_His') . '.csv';
        
        $coupons = Coupon::with('bonusProduct')->get();
        
        $csvContent = '';
        
        // Add headers
        $headers = [
            'Code', 'Name', 'Description', 'Type', 'Value', 'Usage Limit', 'Used Count', 'Usage Display', 
            'User Limit', 'Minimum Order Amount', 'Maximum Discount', 'Is Active', 'Status', 'Starts At', 
            'Expires At', 'Applies To Categories', 'Excluded Products', 'Bonus Product', 'Bonus Quantity', 'Created At'
        ];
        ...
        foreach ($coupons as $coupon) {
            $row = [
                ...
                $coupon->user_limit ?? 'Unlimited',
                $coupon->minimum_order_amount,
                $coupon->maximum_discount,
                $coupon->is_active ? 'Yes' : 'No',
                $coupon->status_with_usage,
                $coupon->starts_at,
                $coupon->expires_at,
                $coupon->applies_to_categories ? json_encode($coupon->applies_to_categories) : '',
                $coupon->excluded_products ? json_encode($coupon->excluded_products) : '',
                $coupon->bonusProduct ? $coupon->bonusProduct->item_name : '',
                $coupon->bonus_quantity,
                $coupon->created_at
            ];
