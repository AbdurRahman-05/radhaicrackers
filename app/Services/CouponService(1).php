...
class CouponService
{
    public function validateCoupon($code, $userId, $orderAmount = 0, $orderItems = [])
    {
        ...
        if ($coupon->usage_limit && $coupon->used_count >= $coupon->usage_limit) {
            \Log::warning('Coupon validation failed: usage limit reached', ['code' => $coupon->code, 'used_count' => $coupon->used_count]);
    ...
    public function applyCouponToOrder($coupon, $order, $orderItems)
    {
        return DB::transaction(function () use ($coupon, $order, $orderItems) {
            ...
            // Auto-inactivate or expire if usage limit reached
            if ($coupon->usage_limit && $coupon->used_count >= $coupon->usage_limit) {
                $coupon->is_active = 0;
